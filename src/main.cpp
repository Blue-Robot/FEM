...
void initializeGPUData(int n) {
	...
	for (int i = 0; i < numVtx; i++) {
		...
		vv_max_size = std::max(vv_size, vv_max_size);
	}

	nbr_v = new uint[numVtx*(vv_max_size+1)]; std::fill_n(nbr_v, numVtx*(vv_max_size+1), 0);
	vertex_weights = new FN_TYPE[numVtx*vv_max_size]; std::fill_n(vertex_weights, numVtx*vv_max_size, 0.0);
	for (int i = 0; i < numVtx; i++) {
		VertexHandle v = orderedMesh.vertex_handle(i);
		...
		for (SimpleTriMesh::VertexOHalfedgeIter heIter= orderedMesh.voh_begin(v); heIter != orderedMesh.voh_end(v); ++heIter) {
			...
			nbr_v[numVtx*(counter+1) + i] = nbr.idx();
			vertex_weights[numVtx*counter + i] = mStats.wHej[heIter.handle().idx()];
			counter++;
		}
		nbr_v[i] = counter;
	}

//	for (int i= 0; i < vv_max_size+1; i++) {
//		for (int j = 0; j < 10; j++) {
//			printf("%d ", nbr_v[numVtx*(i) + j]);
//		}
//		printf("\n");
//	}
...
//	}
	...
	for (int i = 0; i < numVtx; i++) {
		...
		vf_max_size = std::max(vf_size, vf_max_size);
	}

	// fill array
	vertex_faces = new uint[numVtx*(vf_max_size+1)]; std::fill_n(vertex_faces, numVtx*(vf_max_size+1), 0);
	face_weights = new FN_TYPE[numVtx*vf_max_size]; std::fill_n(face_weights, numVtx*vf_max_size, 0.0);
	for (int i = 0; i < numVtx; i++) {
		VertexHandle v = orderedMesh.vertex_handle(i);
		...
		for (SimpleTriMesh::VertexOHalfedgeIter heIter= orderedMesh.voh_begin(v); heIter != orderedMesh.voh_end(v); ++heIter) {
			if (!orderedMesh.is_boundary(heIter.handle())) {
				FaceHandle f = orderedMesh.face_handle(heIter.handle());
				vertex_faces[(counter+1)*numVtx + i] = f.idx();
				face_weights[(counter)*numVtx + i] = mStats.meshAngle[orderedMesh.prev_halfedge_handle(heIter.handle()).idx()];
				counter++;
			}
		}
		vertex_faces[i] = counter;
	}
	...
	for (int i = 0; i < n; i++) {
		...
		if (patch < 2)
			patch+=32;
		hf_max_size = std::max(hf_max_size, (uint)halo_faces_part.size() + patch);
	}

	// fill array
	halo_faces = new uint[n*hf_max_size]; std::fill_n(halo_faces, n*hf_max_size, 0);
	for (int i = 0; i < n; i++) {
		std::set<uint> halo_faces_part;
		for (int j = node_parts[i]; j < node_parts[i+1]; j++) {
			VertexHandle v = orderedMesh.vertex_handle(j);
	...
	face_vertices = new uint[numFaces*3]; std::fill_n(face_vertices, numFaces*3, 0);
	for (int i = 0; i < numFaces; i++) {
		FaceHandle f = orderedMesh.face_handle(i);

		int counter = 0;
		for (SimpleTriMesh::FaceVertexIter fvIter = orderedMesh.fv_begin(f); fvIter != orderedMesh.fv_end(f); ++fvIter) {
			face_vertices[numFaces*counter + i] = fvIter.handle().idx();
			counter++;
		}
	}

	// Set up halfedge gradients
	he_grads = new float3[numFaces*2];
	for (int i = 0; i < numFaces; i++) {
		OpenMesh::Vec3f grad12 = mStats.gradVec12[i];
		he_grads[i] = make_float3(grad12.values_[0], grad12.values_[1], grad12.values_[2]);

		OpenMesh::Vec3f grad13 = mStats.gradVec13[i];
		he_grads[numFaces + i] = make_float3(grad13.values_[0], grad13.values_[1], grad13.values_[2]);
	}
}
...
//	}
	...
	int threads = ((max_size_e + 32 - 1) / 32) * 32;
	//end
	...
	uint *dev_vertex_faces;
	FN_TYPE *dev_face_weights;

	uint *dev_nbr_v;
	FN_TYPE *dev_vertex_weights;

	uint *dev_face_vertices;

	float3 *dev_he_grads;

	size_t hf_pitchInBytes;
	size_t vf_pitchInBytes;
	size_t vv_pitchInBytes;
	size_t fv_pitchInBytes;
	size_t he_pitchInBytes;
	...
	checkCudaErrors(cudaMalloc(&dev_parts_n, sizeof(uint)*(n+2)));
	checkCudaErrors(cudaMalloc(&dev_parts_e, sizeof(uint)*(n+2)));

	checkCudaErrors(cudaMallocPitch(&dev_nbr_v, &vv_pitchInBytes, numVtx*sizeof(uint), vv_max_size+1));
	checkCudaErrors(cudaMallocPitch(&dev_vertex_weights, &vv_pitchInBytes, numVtx*sizeof(FN_TYPE), vv_max_size));

	checkCudaErrors(cudaMallocPitch(&dev_halo_faces, &hf_pitchInBytes, hf_max_size*sizeof(uint), n));
	...
	checkCudaErrors(cudaMallocPitch(&dev_face_vertices, &fv_pitchInBytes, numFaces*sizeof(uint), 3));
	checkCudaErrors(cudaMallocPitch(&dev_he_grads, &he_pitchInBytes, numFaces*sizeof(float3), 2));
	...
	checkCudaErrors(cudaMallocPitch(&dev_vertex_faces, &vf_pitchInBytes, numVtx*sizeof(uint), vf_max_size+1));
	checkCudaErrors(cudaMallocPitch(&dev_face_weights, &vf_pitchInBytes, numVtx*sizeof(uint), vf_max_size));
...
//	checkCudaErrors(cudaMalloc(&dev_faceWeights, sizeof(FN_TYPE)*numFaces*3));
	...
	checkCudaErrors(cudaMemcpy(dev_parts_n, node_parts, sizeof(uint)*(n+2), cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(dev_parts_e, element_parts, sizeof(uint)*(n+2), cudaMemcpyHostToDevice));

	checkCudaErrors(cudaMemcpy2D(dev_nbr_v, vv_pitchInBytes, nbr_v, numVtx*sizeof(uint), numVtx*sizeof(uint), vv_max_size+1, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy2D(dev_vertex_weights, vv_pitchInBytes, vertex_weights, numVtx*sizeof(FN_TYPE), numVtx*sizeof(FN_TYPE), vv_max_size, cudaMemcpyHostToDevice));

	checkCudaErrors(cudaMemcpy2D(dev_halo_faces, hf_pitchInBytes, halo_faces, hf_max_size*sizeof(uint), hf_max_size*sizeof(uint), n, cudaMemcpyHostToDevice));
	...
	checkCudaErrors(cudaMemcpy2D(dev_face_vertices, fv_pitchInBytes, face_vertices, numFaces*sizeof(uint), numFaces*sizeof(uint), 3, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy2D(dev_he_grads, he_pitchInBytes, he_grads, numFaces*sizeof(float3), numFaces*sizeof(float3), 2, cudaMemcpyHostToDevice));
	...
	checkCudaErrors(cudaMemcpy2D(dev_vertex_faces, vf_pitchInBytes, vertex_faces, numVtx*sizeof(uint), numVtx*sizeof(uint), vf_max_size+1, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy2D(dev_face_weights, vf_pitchInBytes, face_weights, numVtx*sizeof(uint), numVtx*sizeof(uint), vf_max_size, cudaMemcpyHostToDevice));
...
//	checkCudaErrors(cudaMemcpy(dev_faceWeights, faceWeights, sizeof(FN_TYPE)*numFaces*3, cudaMemcpyHostToDevice));
	...
	cudaProfilerStart();
	step(dev_nFn_one, dev_cFn_one, dev_nFn_two, dev_cFn_two, dev_face_vertices, fv_pitchInBytes, dev_nbr_v, dev_vertex_weights, vv_pitchInBytes, dev_vtxW, dev_he_grads, he_pitchInBytes, dev_nFaceGradients, dev_cFaceGradients, dev_vertex_faces, dev_face_weights, vf_pitchInBytes, dev_parts_n, dev_parts_e, dev_halo_faces, hf_pitchInBytes, n, threads, dt);
	cudaProfilerStop();
	step(dev_nFn_two, dev_cFn_two, dev_nFn_one, dev_cFn_one, dev_face_vertices, fv_pitchInBytes, dev_nbr_v, dev_vertex_weights, vv_pitchInBytes, dev_vtxW, dev_he_grads, he_pitchInBytes, dev_nFaceGradients, dev_cFaceGradients, dev_vertex_faces, dev_face_weights, vf_pitchInBytes, dev_parts_n, dev_parts_e, dev_halo_faces, hf_pitchInBytes, n, threads, dt);
