...
__global__ void stepKernel(float2 *fn_src, float2 *fn_dst, uint *halo_sizes, uint *halo_vertices, uint hv_pitch,
	...
	float2 *s_fn = (float2 *)&s_wg[size+size%2];
	for (int i = threadIdx.x; i < 7*size; i += blockDim.x) {
		s_mem[i] = 0.0;
	}
	if (threadIdx.x < size)
		s_fn[threadIdx.x] = fn_src[vertex_parts[blockIdx.x] + threadIdx.x];

	if(threadIdx.x < halo_sizes[blockIdx.x])
		s_fn[threadIdx.x+size] = fn_src[halo_vertices[blockIdx.x*hv_pitch + threadIdx.x]];
	...
	float2 v1 = s_fn[fn_index[2]];
	float2 v12 = s_fn[fn_index[0]] - v1;
	float2 v13 = s_fn[fn_index[1]] - v1;
	...
	float2 lap = s_fn[threadIdx.x] * vW;
	...
	/* update *********************************************/
	FN_TYPE dauN = D * lap.x - alpha * s_fn[threadIdx.x].x * lap.y - alpha * dotP
			+ S * r * s_fn[threadIdx.x].x * (nMax - s_fn[threadIdx.x].x);
	FN_TYPE dauC = lap.y + S * (s_fn[threadIdx.x].x / (1 + s_fn[threadIdx.x].x) - s_fn[threadIdx.x].y);

	fn_dst[i].x = dt * dauN + s_fn[threadIdx.x].x;
	fn_dst[i].y = dt * dauC + s_fn[threadIdx.x].y;
}

extern "C" void step(float2 *fn_src, float2 *fn_dst, uint *dev_halo_sizes, uint *halo_vertices, uint hv_pitchInBytes,
	...
	stepKernel<<<grid, block, smem_size>>>(fn_src, fn_dst, dev_halo_sizes, halo_vertices, hv_pitchInBytes/sizeof(uint),
